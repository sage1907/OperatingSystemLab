...
int main()
{
    ...
    if (pid == 0)
    {
        close(pipefds[1]); // Close the write end of the pipe in the parent (not required)
        printf("\n\n\nIn the child process: Dequeue\n\n\n");
        ...
        close(pipefds[0]); // Close the read end of the pipe in the parent (read complete)
    }
    else
    {
        close(pipefds[0]); // Close the read end of the pipe in the child (not required)
        printf("\n\n\nIn the parent process: Enqueue\n\n\n");
        while (1)
        {
            getUserInput(&buf);
        ...
        close(pipefds[1]); // Close the write end of the pipe in the child (write complete)
        exit(0);
    }
