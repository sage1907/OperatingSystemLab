...
int main()
{
    ...
    *status = 1;
    ...
    int c = semctl(semid, 1, SETVAL, setvalArg);
    if (c == -1)
    {
    ...
    // setvalArg.val = 0;
    // int semctl(int semid, int semnum, int cmd, ...);
    c = semctl(semid, 2, SETVAL, setvalArg);
    if (c == -1)
    {
        perror("semctl() failed");
        exit(1);
    ...
    setvalArg.val = 1;
    c = semctl(semid, 0, SETVAL, setvalArg);
    if (c == -1)
    {
        perror("semctl() failed");
        exit(1);
    ...
    while (*status)
    {
        // int semctl(int semid, int semnum, int cmd, ...);
        Pop.sem_num = 0;
        c = semctl(semid, 0, GETVAL, setvalArg);
        if (c == -1)
        {
            perror("semctl() failed");
            exit(1);
        ...
        P(semid);
        fprintf(stdout, "I am p1!\n");
        Vop.sem_num = 1;
        int c = semctl(semid, 1, GETVAL, setvalArg);
        if (c == -1)
        {
            perror("semctl() failed");
            exit(1);
