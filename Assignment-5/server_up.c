...
void release_shm(int signum) {
    ...
    if (status == 0) {
        ...
        fprintf(stderr, "kill(2) returned the wrong value\n");
    }
}
...
int main() {
    ...
    while (task_ptr->status != -1) {
        if (task_ptr->status == 0) {
            ...
            printf("Task created with task_id %d and unsorted data: ", task_ptr->task_id);
            for (int i = 0; i < ARRAY_SIZE; i++) {
            ...
            task_ptr->status = 4;
            printf("Task with task_id %d sorted by worker with PID %d\n", task_ptr->task_id, task_ptr->worker_pid);
            printf("Sorted data for task_id %d: ", task_ptr->task_id);
